...
const Index = () => {
  ...
  const fetchCandidates = async (numbersArr: number[]) => {
    ...
    try {
      ...
      const formattedResults = scoreData.results.map((c: any, i: number) => {
        ...
        let statusText = "Not Qualified";
        if (atsScore > qualifiedThresholdDynamic) statusText = "Qualified";
        else if (atsScore > reviewThresholdDynamic) statusText = "Review";

        const videoInterview = c?.["Video Interview"] || {};
        return {
          id: i + 1,
          name: c?.Name || "Unknown",
          ...
          videoInterviewStatus: videoInterview.Status || "Pending",
          exprHappy: videoInterview.Happy || 0,
          exprNeutral: videoInterview.Neutral || 0,
          exprSad: videoInterview.Sad || 0,
          exprAngry: videoInterview.Angry || 0,
          dominant: videoInterview.Dominant || "Neutral",
          shortlisted: false,
        };
      });
  ...
  const simulateProcessing = async (numbersArr: number[]) => {
    await fetchCandidates(numbersArr);
  };
  ...
  const handleExcelDownload = () => {
    const csvContent =
      ...
      "Name,Email,Phone,ATS Score,Key Strength,Considerations,Status,Video Status,Happy %,Neutral %,Sad %,Angry %,Dominant,Shortlisted\n" +
      results
        .map(
          (r) =>
            `${r.name},${r.email},${r.phone},${r.atsScore},${r.KeyStrength},${
              r.considerations
            },${r.status},${r.videoInterviewStatus},${r.exprHappy},${
              r.exprNeutral
            },${r.exprSad},${r.exprAngry},${r.dominant},${
                  ...
                  {results.map((candidate) => (
                    <TableRow key={candidate.id} className="hover:bg-blue-50">
                      ...
                      <TableCell>
                        ...
                        candidate.atsScore > 10 ? (
                          <a
                            className="underline text-blue-600"
                            href={`/interview?email=${
                              candidate.email
                            }&name=${encodeURIComponent(candidate.name)}`}
                          >
                            Start
                          </a>
                        ) : (
                      ...
                      <TableCell>{candidate.dominant}</TableCell>
                      <TableCell>
                        <Checkbox
